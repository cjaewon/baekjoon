...
방향 이동 -> 그냥 나머지 연산을 써서 해결하자.

1...12 까지 있다하면
그냥 % 12하면 됨. 다만 x = x % 12를 매번 해서 x가 무작정 커지는 걸 방지해야함
왜냐면 2 x (1 <= x <= 10 ** 9) 이기 떄문에.

예외처리가 필요하네 % N == 0 이면 pos = N으로

---
match case를 적극적으로 써보자.
"""
...
ordered_set = TreapSet([i + 1 for i in range(N) if places[i] == 1])
pos = 1

for _ in range(Q):
  match list(map(int, input().split())):
    case [1, i]:
      k = ordered_set.ceiling(i)
    ...
    case [2, x]:
      ...
      if pos == 0:
        pos = N
    case [3]:
      celiling_next = ordered_set.ceiling(pos)
    
      try:
        left_next = ordered_set.min()
      except ValueError:
        left_next = None

      if celiling_next:
        print(celiling_next - pos)
      elif left_next and left_next < pos:
        print((N - pos) + left_next)
      else:
        print(-1)